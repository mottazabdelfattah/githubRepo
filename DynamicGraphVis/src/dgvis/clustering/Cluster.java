...
public class Cluster
{
    ...
    public Cluster copy(){
    	...
    	Cluster right = null;
    	if(!this.isLeaf()){
    		left = this.getChildren().get(0).copy();
    		right = this.getChildren().get(1).copy();
    	}
    	Cluster c = new Cluster(this.name);
    	if(left != null)
    		c.addChild(left);
    	if(right != null)
    		c.addChild(right);
    ...
    public List<String> getClusterLeafsByTraversing(){
    	// get leafs
    	
    	List<String> leafs = new ArrayList<String>();
    	
		Stack<Cluster> clusterStack = new Stack<Cluster>();
		clusterStack.add(this);
		while(!clusterStack.isEmpty()){
			Cluster tCluster = clusterStack.pop();
			if(tCluster.isLeaf()){
				leafs.add(tCluster.getName());
			}else{
				for(int c = tCluster.getChildren().size()-1; c>=0 ; c--){
					clusterStack.push(tCluster.getChildren().get(c));
				}
			}
		}
